...
def modify_python_file(task, filename, output):
    ...
    for line in lines:
        ...
        if stripped.startswith("#"):
            # Jason (7/17): Keep commenting for torch.jit.script functions
        ...
        # if "return total_reward," in line:
        #     line = line.replace("return total_reward,", "return")
...
tasks = [
    ...
    # ["ingenuity", "ingenuity"],
    # ["franka", "franka_cube_stack"],
    # ["franka", "franka_cabinet"]
    # ["hand", "shadow_hand_free_arm"],
    # ["hand", "shadow_hand_upside_down"],
    ["hand", "shadow_hand_spin"],
]

for task_name, task in tasks:

    # Create base environment file to write reward function for
    modify_python_file(task_name, f"tasks/{task}.py", f"tasks/isaac_gpt_base/{task}.py")
    
    # Create a condensed version to serve as input to GPT
    prune_python_class(f"tasks/isaac_gpt_base/{task}.py", f"tasks/isaac_gpt_base/{task}_obs.py", 
