...
def main(cfg):
    ...
    if cfg.no_verbalization:
        with open(f'{ROOT_DIR}/gpt_utils/prompts/policy_feedback.txt', 'r') as file:
            policy_feedback = file.read()
    else:
        with open(f'{ROOT_DIR}/gpt_utils/prompts/policy_feedback_no_verbalization.txt', 'r') as file:
            policy_feedback = file.read()
    ...
    for iter in range(cfg.iteration):
        ...
        if cfg.self_critic:
                        ...
                        for metric in tensorboard_logs:
                            if "/" not in metric:
                                ...
                                if metric != "gt_reward" and metric != "gpt_reward":
                                    ...
                                    if cfg.no_verbalization and (metric_name != "success_rate" and metric_name != "episode_lengths"):
                                        continue
            ...
            # # Optionally, use multiple reward  
            # if cfg.top_k != 0 and np.sum(np.array(successes) >= 0.) >= cfg.top_k:
            #     content_overall = f"We trained RL policies using the following {top_k} reward function code and track the values of the individual components in the reward functions as well as global policy metrics such as success rates and episode lengths after every {epoch_freq} epochs and the maximum, mean, minimum values encountered."
            #     top_k = np.array(successes).argsort()[-cfg.top_k:][::-1]
            #     for index in top_k:
            #         reward_k = code_runs[index]
            #         feedback_k = code_feedbacks[index]
            #         content_k = f"Reward Function {index}: {reward_k}"
            #         content_k += policy_feedback.format(epoch_freq=epoch_freq)
            #         content_k += feedback_k
