...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Calculate the orientation difference between object rotation and goal orientation
    q_diff = torch.mul(object_rot, -1.0) * goal_rot
    q_diff_scaled = q_diff / torch.norm(q_diff, dim=-1, keepdim=True)
    
    # Calculate the angular difference, which lies between -1 and 1
    cos_diff = 2.0 * torch.pow(q_diff_scaled[..., 0], 2) - 1.0
    
    # Apply a transformation to the angular difference, using a temperature parameter inside the transformation function
    angular_diff_temperature = torch.tensor(10.0, device=cos_diff.device)
    reward = torch.exp(angular_diff_temperature * cos_diff) - 1.0

    # Create a dictionary with the individual reward component
    reward_components = {"angular_difference": reward}
