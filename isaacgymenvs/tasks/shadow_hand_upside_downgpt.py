...
class ShadowHandUpsideDownGPT(VecTask):
...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, fingertip_pos: torch.Tensor, object_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Distance between the object orientation and the goal orientation
    q_diff = torch.abs(torch.sum(object_rot * goal_rot, dim=-1) - 1.0)
    orientation_reward_temperature = 10.0  # Increase the temperature for orientation_reward
    orientation_reward = torch.exp(-orientation_reward_temperature * q_diff)

    # Penalize the distance of the fingertips to the object, with a reduced temperature parameter to focus on orientation reward
    fingertip_distance_temperature = 2.0
    fingertip_distance = torch.mean(torch.norm(fingertip_pos - object_pos.unsqueeze(1), dim=-1), dim=1)
    fingertip_penalty = torch.exp(-fingertip_distance_temperature * fingertip_distance)

    # Combine both rewards with a multiplier to emphasize the orientation_reward and maintain the grasp of the object
    reward_multiplier = 0.95
    total_reward = orientation_reward * (reward_multiplier * fingertip_penalty + (1 - reward_multiplier))

    # Individual reward components
    reward_components = {"orientation_reward": orientation_reward, "fingertip_penalty": fingertip_penalty}
