...
class ShadowHandOverGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.fingertip_pos, self.fingertip_another_pos) 
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, fingertip_pos: torch.Tensor,
                   fingertip_another_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    distance_weight = 1.0
    reach_goal_weight = 10.0
    touch_goal_weight = 5.0
    temp_distance = 0.1
    temp_reach_goal = 0.1
    temp_touch_goal = 0.1

    goal_reached = torch.sqrt(torch.sum((object_pos - goal_pos)**2, dim=1)) < 0.05
    goal_reached = goal_reached.to(torch.float32)
    
    fingertip_distances = torch.sqrt(torch.sum((fingertip_pos - goal_pos.unsqueeze(1))**2, dim=2))
    fingertip_touch_goal = torch.min(fingertip_distances, dim=1).values < 0.05

    fingertip_another_distances = torch.sqrt(torch.sum((fingertip_another_pos - goal_pos.unsqueeze(1))**2, dim=2))
    fingertip_another_touch_goal = torch.min(fingertip_another_distances, dim=1).values < 0.05

    all_fingertips_touch_goal = torch.logical_and(fingertip_touch_goal, fingertip_another_touch_goal)

    dist_reward = -torch.exp(-(torch.min(fingertip_distances, dim=1).values) / temp_distance) * distance_weight
    reach_goal_reward = torch.exp(-goal_reached / temp_reach_goal) * reach_goal_weight
    touch_goal_reward = torch.exp(-all_fingertips_touch_goal.to(torch.float32) / temp_touch_goal) * touch_goal_weight

    total_reward = dist_reward + reach_goal_reward + touch_goal_reward

    reward_info = {
        "dist_reward": dist_reward,
        "reach_goal_reward": reach_goal_reward,
        "touch_goal_reward": touch_goal_reward
    }
