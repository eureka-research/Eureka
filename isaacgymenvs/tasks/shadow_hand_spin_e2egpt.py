...
class ShadowHandSpinE2EGPT(VecTask):
...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    rotation_error = torch.abs(torch.sum(torch.mul(goal_rot, object_rot), dim=1) - 1)

    # Normalize the rotation error to a fixed range using the exponential function
    rotation_temperature = 10.0
    normalized_rotation_error = torch.exp(-rotation_temperature * rotation_error)

    # Compute the total reward as the negative of the rotation error
    reward = 1.0 - normalized_rotation_error

    # Return the total reward and a dictionary containing the individual reward components
    return reward, {'rotation_error': normalized_rotation_error}
