...
class CartpoleGPT(VecTask):
    ...
    def compute_reward(self):
        self.rew_buf[:], self.rew_dict = compute_reward(self.obs) 
...
@torch.jit.script
def compute_reward(obs: Tensor) -> Tuple[Tensor, Dict[str, Tensor]]:
    cart_position = obs[:, 0]
    cart_velocity = obs[:, 1]
    pole_angle = obs[:, 2]
    pole_velocity = obs[:, 3]
    ...
    angle_reward_coeff = 1.0
    angle_penalty_coeff = 5.0
    dist_reward_coeff = 1.0
    dist_penalty_coeff = 1.0
    cart_vel_coeff = 1.0
    pole_vel_coeff = 1.0

    # The angle reward should be maximum when the pole is upright and minimized when it deviates
    angle_reward = angle_reward_coeff * torch.cos(pole_angle)
    angle_penalty = angle_penalty_coeff * torch.abs(pole_angle)

    # The distance reward should be maximum when the cart is near center and minimized when it deviates
    dist_reward = dist_reward_coeff * torch.ones(obs.size(0), dtype=obs.dtype, device=obs.device) - torch.abs(cart_position)
    dist_penalty = dist_penalty_coeff * torch.abs(cart_position)

    # Penalize cart and pole velocities to keep them steady
    cart_vel_penalty = cart_vel_coeff * torch.abs(cart_velocity)
    pole_vel_penalty = pole_vel_coeff * torch.abs(pole_velocity)

    # Calculate the total reward by summing up all rewards and penalties
    total_reward = angle_reward - angle_penalty + dist_reward - dist_penalty - cart_vel_penalty - pole_vel_penalty
