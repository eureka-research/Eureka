...
class FrankaCubeStackGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.eef_pos, self.cubeA_pos, self.cubeB_pos, self.cubeA_to_cubeB_pos) 
...
@torch.jit.script
def compute_reward(eef_pos: torch.Tensor, cubeA_pos: torch.Tensor, cubeB_pos: torch.Tensor, cubeA_to_cubeB_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Compute the distance between the end-effector (eef) and Cube A
    eef_to_cubeA_distance = torch.norm(eef_pos - cubeA_pos, dim=-1)
    
    # Compute the distance between Cube A and Cube B
    cubeA_to_cubeB_distance = torch.norm(cubeA_to_cubeB_pos, dim=-1)
    
    # Define the target distance for Cube A to be placed on top of Cube B
    target_distance = 0.02

    # Reward for being close to Cube A
    eef_to_cubeA_reward_temperature = 0.1
    eef_to_cubeA_reward = torch.exp(-eef_to_cubeA_distance / eef_to_cubeA_reward_temperature)

    # Reward for stacking Cube A on top of Cube B
    cubeA_to_cubeB_reward_temperature = 0.1
    cubeA_to_cubeB_reward = torch.exp(-(cubeA_to_cubeB_distance - target_distance) / cubeA_to_cubeB_reward_temperature)

    # Calculate the total reward
    total_reward = eef_to_cubeA_reward + cubeA_to_cubeB_reward

    # Create a dictionary for the individual reward components
    reward_components = {
        "eef_to_cubeA_reward": eef_to_cubeA_reward,
        "cubeA_to_cubeB_reward": cubeA_to_cubeB_reward,
    }
