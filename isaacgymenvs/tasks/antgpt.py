...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_task3(self.roll, self.pitch) 
...
@torch.jit.script
def compute_reward_task3(roll: torch.Tensor, pitch: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    temp_orient = 1.5
    
    roll_pen = torch.abs(roll)
    pitch_pen = torch.abs(pitch)
    
    orient_penalty = roll_pen + pitch_pen
    
    # Apply transformation to reward component
    transformed_orient_penalty = torch.exp(-temp_orient * orient_penalty)

    reward = transformed_orient_penalty
    reward_info = {'roll_penalty': roll_pen, 'pitch_penalty': pitch_pen, 'orientation_penalty': transformed_orient_penalty}
