...
class ShadowHandThreeBallJuggle(VecTask):
    """
    This class corresponds to the ThreeBallJuggle task.
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object3_asset = self.gym.load_asset(self.sim, asset_root, object_asset_file, object_asset_options)
        ...
        if False:  # For hands facing each other (like ShadowHandCatchUnderarm)
            ...
            object3_start_pose = gymapi.Transform()
            object3_start_pose.p = object2_start_pose.p
            object3_start_pose.r = object2_start_pose.r
            object3_start_pose.p.z += 0.3
        ...
        for i in range(self.num_envs):
            ...
            object3_handle = self.gym.create_actor(env_ptr, object3_asset, object3_start_pose, "object3", i, 0, 0)
            self.object3_init_state.append([object3_start_pose.p.x, object3_start_pose.p.y, object3_start_pose.p.z,
                                           object3_start_pose.r.x, object3_start_pose.r.y, object3_start_pose.r.z, object3_start_pose.r.w,
                                           0, 0, 0, 0, 0, 0])
            object3_idx = self.gym.get_actor_index(env_ptr, object3_handle, gymapi.DOMAIN_SIM)
            ...
            if self.object_type != "block":
                ...
                self.gym.set_rigid_body_color(
                    env_ptr, object3_handle, 0, gymapi.MESH_VISUAL, gymapi.Vec3(0.72, 0.98, 0.6))
    ...
    def compute_reward(self, actions):
        ...
        Compute the reward of all environment. The core function is compute_hand_reward(
            ...
            self.max_episode_length, self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.object2_pos, self.object2_rot, self.goal_another_pos, self.goal_another_rot,
        ...
        #     self.rew_buf, self.reset_buf, self.reset_goal_buf, self.progress_buf, self.successes, self.consecutive_successes,
        #     self.max_episode_length, self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions, self.action_penalty_scale,
        #     self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
        #     self.max_consecutive_successes, self.av_factor, (self.object_type == "pen")
        # )
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        the state values of the left and right hands, and the information of objects. 
        ...
        411 - 417   object2 pose
        418 - 420   object2 linear velocity
        421 - 423   object2 angle velocity
        424 - 430   object3 pose
        431 - 433   object3 linear velocity
        434 - 436   object3 angle velocity
        """
        ...
        obj3_obs_start = obj2_obs_start + 13
        self.obs_buf[:, obj3_obs_start:obj3_obs_start + 7] = self.object3_pose
        self.obs_buf[:, obj3_obs_start + 7:obj3_obs_start + 10] = self.object3_linvel
        self.obs_buf[:, obj3_obs_start + 10:obj3_obs_start + 13] = self.vel_obs_scale * self.object3_angvel
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        self.root_state_tensor[self.goal_object2_indices[env_ids], 0:3] = self.goal_another_states[env_ids, 0:3] + self.goal_displacement_tensor
        self.root_state_tensor[self.goal_object2_indices[env_ids], 3:7] = self.goal_another_states[env_ids, 3:7]
        self.root_state_tensor[self.goal_object2_indices[env_ids], 7:13] = torch.zeros_like(self.root_state_tensor[self.goal_object2_indices[env_ids], 7:13])

        if apply_reset:
            goal_object_indices = self.goal_object_indices[env_ids].to(torch.int32)
            goal_object2_indices = self.goal_object2_indices[env_ids].to(torch.int32)
            self.gym.set_actor_root_state_tensor_indexed(self.sim,
                                                         gymtorch.unwrap_tensor(self.root_state_tensor),
                                                         gymtorch.unwrap_tensor(goal_object_indices), len(env_ids))
            self.gym.set_actor_root_state_tensor_indexed(self.sim,
                                                         gymtorch.unwrap_tensor(self.root_state_tensor),
                                                         gymtorch.unwrap_tensor(goal_object2_indices), len(env_ids))
    ...
    def reset_idx(self, env_ids, goal_env_ids):
        ...
        self.root_state_tensor[self.object3_indices[env_ids], 3:7] = new_object_rot
        ...
        object_indices = torch.unique(torch.cat([self.object1_indices[env_ids],
                                                 self.object2_indices[env_ids],
                                                 self.object3_indices[env_ids]]).to(torch.int32))
...
def compute_hand_reward(
    ...
    Args:
        ...
        object2_pos (tensor): The position of the another object

        object2_rot (tensor): The rotation of the another object
    ...
    goal_another_dist = torch.norm(target_another_pos - object2_pos, p=2, dim=-1)
    if ignore_z_rot:
        success_tolerance = 2.0 * success_tolerance
    ...
    quat_another_diff = quat_mul(object2_rot, quat_conjugate(target_another_rot))
    ...
    reward = torch.where(object2_pos[:, 2] <= 0.2, reward + fall_penalty, reward)

    # Check env termination conditions, including maximum success number
    resets = torch.where(object_pos[:, 2] <= 0.2, torch.ones_like(reset_buf), reset_buf)
    resets = torch.where(object2_pos[:, 2] <= 0.2, torch.ones_like(reset_buf), resets)
