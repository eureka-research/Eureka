...
import torch
@torch.jit.script
def compute_bbot_reward(tray_positions: torch.Tensor, ball_positions: torch.Tensor, ball_linvels: torch.Tensor,
                        ball_radius: float, reset_buf: torch.Tensor, progress_buf: torch.Tensor,
                        max_episode_length: int) -> Tuple[torch.Tensor, torch.Tensor]:
    num_envs = tray_positions.shape[0]

    # Calculate the difference between the tray and ball positions
    pos_delta = ball_positions - tray_positions

    # Compute the distance from the ball to the tray center
    ball_distance = pos_delta.norm(p=2, dim=-1)

    # Calculate the height difference between the ball and the tray
    height_diff = pos_delta[..., 2]

    # Calculate stability as the inverse of the distance and height difference
    stability = 1.0 / (1.0 + ball_distance + height_diff)

    # Calculate the horizontal speed of the ball
    ball_hor_speed = ball_linvels[..., 0:2].norm(p=2, dim=-1)

    # Weight the stability and horizontal speed for better control
    reward = 0.9 * stability - 0.1 * ball_hor_speed

    # Check if the ball falls off the tray
    fallen_off = height_diff < -ball_radius

    # Set the reset buffer to 1 for the environments where the ball has fallen off
    reset_buf[fallen_off.nonzero()] = 1.0

    # Check if we exceed the maximum time steps
    episode_end = progress_buf >= max_episode_length

    # Set reset buffer to 1 for those environments
    reset_buf[episode_end.nonzero()] = 1.0
