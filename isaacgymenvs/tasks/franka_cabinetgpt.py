...
@torch.jit.script
def compute_franka_reward(reset_buf, progress_buf, actions, cabinet_dof_pos,
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor) -> Tuple[Tensor, Tensor]

    # Reward for moving the gripper closer to the drawer handle
    dist_reward = -torch.norm(franka_grasp_pos - drawer_grasp_pos, dim=-1)

    # Reward for aligning the gripper with the handle
    alignment_reward = torch.sum(gripper_forward_axis * drawer_inward_axis, dim=-1) + torch.sum(gripper_up_axis * drawer_up_axis, dim=-1)
    ...
    reward = dist_reward + alignment_reward + open_reward + action_penalty
