...
@torch.jit.script
def reward_backflip(position: torch.Tensor, target_pos: torch.Tensor, angle_diff: torch.Tensor, target_angle: torch.Tensor) -> Tuple[
    torch.Tensor, Dict[str, torch.Tensor]]:
    pos_diff = position - target_pos
    distance = torch.linalg.norm(pos_diff, dim=-1)
    angle_distance = torch.abs(torch.atan2(angle_diff[1], angle_diff[0]))
    reward = -distance - angle_distance
    reward_components = {'distance': -distance, 'angle_distance': -angle_distance}
