...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        ...
        # self.rew_buf[:], self.rew_dict = compute_reward(self.object_angvel, self.object_rot)
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):
        ...
        # self.goal_states[:, 3] = 0.576
        # self.goal_states[:, 4] = -0.204
        # self.goal_states[:, 5] = 0.681
        # self.goal_states[:, 6] = 0.404
        # self.goal_states[:, :] = quat_unit(self.goal_states[:, :])

        # temp = self.goal_states[:1, 3:7].to(device="cpu")
        # vec = torch.Tensor([[0, 0, 1]])
        # print(quat_rotate(temp, vec))
