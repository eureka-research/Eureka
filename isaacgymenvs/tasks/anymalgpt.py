...
class AnymalGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.base_pos, self.base_quat, self.target_pos, self.target_quat) 
...
@torch.jit.script
def compute_reward(base_pos: torch.Tensor, base_quat: torch.Tensor, target_pos: torch.Tensor, 
                   target_quat: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    pos_dist = torch.norm(base_pos - target_pos, dim=-1)  # distance between robot base position and target position
    quat_distance = 1.0 - torch.abs(torch.matmul(base_quat, target_quat.unsqueeze(-1)).squeeze(-1))  # quaternion distance between current and target orientations
    reward = torch.exp(-pos_dist - quat_distance)
    return reward, {"distance_reward": torch.exp(-pos_dist), "quaternion_distance_reward": torch.exp(-quat_distance)}
