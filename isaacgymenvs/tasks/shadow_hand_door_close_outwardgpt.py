...
class ShadowHandDoorCloseOutwardGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.door_left_handle_pos, self.door_right_handle_pos, self.left_hand_pos, self.right_hand_pos, self.goal_pos) 
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, door_left_handle_pos: torch.Tensor, door_right_handle_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    dist_to_object = torch.norm(left_hand_pos - object_pos, dim=1)
    reach_reward = torch.exp(-dist_to_object * 3.0) / (torch.exp(-dist_to_object * 3.0) + 1.0)

    # Compute grasp reward
    dist_to_left_handle = torch.norm(left_hand_pos - door_left_handle_pos, dim=1)
    dist_to_right_handle = torch.norm(right_hand_pos - door_right_handle_pos, dim=1)
    grasp_reward = torch.exp(-dist_to_left_handle * 5.0) / (torch.exp(-dist_to_left_handle * 5.0) + torch.exp(-dist_to_right_handle * 5.0))

    # Compute door reward
    door_reward = torch.exp(-torch.min(dist_to_left_handle, dist_to_right_handle) * 5.0)

    # Compute goal reward
    dist_to_goal = torch.norm(left_hand_pos - goal_pos, dim=1)
    goal_reward = torch.exp(-dist_to_goal * 5.0) / (torch.exp(-dist_to_goal * 5.0) + 1.0)

    # Compute total reward
    total_reward = reach_reward + grasp_reward + door_reward + goal_reward
