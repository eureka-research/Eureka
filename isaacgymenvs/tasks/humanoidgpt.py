...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.potentials, self.prev_potentials, self.dt) 
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Calculate potential progress through the distance, normalized by time
    current_potentials = -torch.norm(to_target, p=2, dim=-1) / dt
    potential_progress = potentials - prev_potentials

    # Calculate forward velocity
    forward_velocity = -velocity[:, 0]

    # Calculate vertical velocity penalty
    vertical_velocity = velocity[:, 2]
    vertical_penalty = torch.abs(vertical_velocity)

    # Define reward weights and temperature
    potential_weight = 1.0
    potential_temperature = 1.0
    forward_weight = 0.5
    forward_temperature = 1.0
    vertical_weight = -0.5  # to reduce hopping
    vertical_temperature = 1.0

    # Compute reward components
    potential_reward = torch.exp(potential_weight * potential_progress / potential_temperature)
    forward_reward = torch.exp(forward_weight * forward_velocity / forward_temperature)
    vertical_penalty_reward = torch.exp(vertical_weight * vertical_penalty / vertical_temperature)

    # Calculate total reward
    total_reward = potential_reward + forward_reward + vertical_penalty_reward

    # Return total reward and individual reward components in a dictionary
    reward_components = {
        "potential_reward": potential_reward,
        "forward_reward": forward_reward,
        "vertical_penalty_reward": vertical_penalty_reward,
    }
