...
class ShadowHandTwoBallJuggleGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object1_pos, self.object1_linvel, self.object2_pos, self.object2_linvel, self.fingertip_pos, self.fingertip_another_pos) 
...
@torch.jit.script
def compute_reward(object1_pos: torch.Tensor, object1_linvel: torch.Tensor, object2_pos: torch.Tensor, object2_linvel: torch.Tensor, fingertip_pos: torch.Tensor, fingertip_another_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define reward components
    # Reward for keeping the balls in the air (higher position is better)
    height_object1 = object1_pos[:, 1]
    height_object2 = object2_pos[:, 1]
    height_reward = height_object1 + height_object2
    
    # Velocity reward to make sure the balls are moving upward
    velocity_reward_object1 = torch.clamp(object1_linvel[:, 1], min=0)
    velocity_reward_object2 = torch.clamp(object2_linvel[:, 1], min=0)
    velocity_reward = velocity_reward_object1 + velocity_reward_object2

    # Distance reward to encourage balls to stay close to the fingertips
    distance_object1_fingertip = torch.norm(object1_pos - fingertip_pos, dim=1)
    distance_object2_fingertip = torch.norm(object2_pos - fingertip_another_pos, dim=1)
    distance_reward = -1 * (distance_object1_fingertip + distance_object2_fingertip)
    ...
    height_transformed = torch.exp(height_temp * height_reward)
    velocity_transformed = torch.exp(velocity_temp * velocity_reward)
    distance_transformed = torch.exp(distance_temp * distance_reward)

    total_reward = height_weight * height_transformed + velocity_weight * velocity_transformed + distance_weight * distance_transformed

    # Return total reward and individual reward components
    reward_info = {'height_reward': height_reward, 'velocity_reward': velocity_reward, 'distance_reward': distance_reward}
