...
ITERATION=1
MODEL=gpt-4-0314
JOB_NAME='reward-gpt4-final-isaac-markov-gtfeedback-32batch'
case $1 in
    -a)
        ...
        ;;
    -b)
        python gpt_bidex.py --multirun env=franka_cabinet,allegro_hand temperature=$TEMPERATURE sample=$SAMPLE iteration=$ITERATION model=$MODEL hydra/output=ngc hydra.job.name=$JOB_NAME
        python gpt_bidex.py --multirun env=franka_cabinet,allegro_hand temperature=$TEMPERATURE sample=$SAMPLE iteration=$ITERATION model=$MODEL hydra/output=ngc hydra.job.name=$JOB_NAME
        python gpt_bidex.py --multirun env=franka_cabinet,allegro_hand temperature=$TEMPERATURE sample=$SAMPLE iteration=$ITERATION model=$MODEL hydra/output=ngc hydra.job.name=$JOB_NAME
        python gpt_bidex.py --multirun env=franka_cabinet,allegro_hand temperature=$TEMPERATURE sample=$SAMPLE iteration=$ITERATION model=$MODEL hydra/output=ngc hydra.job.name=$JOB_NAME
        python gpt_bidex.py --multirun env=franka_cabinet,allegro_hand temperature=$TEMPERATURE sample=$SAMPLE iteration=$ITERATION model=$MODEL hydra/output=ngc hydra.job.name=$JOB_NAME
